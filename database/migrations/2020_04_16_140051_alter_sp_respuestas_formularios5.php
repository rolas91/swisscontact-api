...
class AlterSpRespuestasFormularios5 extends Migration
{
    public function up()
    {
        $sp_sql =<<<SQL
DROP PROCEDURE IF EXISTS sp_respuestas_formulario;
CREATE PROCEDURE `sp_respuestas_formulario`(in `pPage` int, in `pRowsPerPage` int,IN `pId` INT, IN `pCentros` TEXT)
BEGIN
...
case when p.id is not null then p.correo else null end as correo_participante,
    ...
    case when p.id is not null then p.fecha_nacimiento else null end as fecha_nacimiento_participante,
        ...
        case when p.id is not null then p.salario else null end as salario_participante,
                    ...
                    case when p.id is not null then TIMESTAMPDIFF(YEAR, p.fecha_nacimiento, CURDATE()) else null end as edad
                    ,count(1) as contador, cast(cursos.costo as int) as costo,',@sql,'
...
set @sql = concat('select *,case when edad between 12 and 21 then \'12-21\'
    ...
    -- En esta parte ordenamos y obtenemos el total de rows
